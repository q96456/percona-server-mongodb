...
void OpObserverImpl::aboutToInserts(OperationContext* txn,
    ...
    for (auto it = begin; it != end; it++) {
        if (css->isDocumentInMigratingChunk(txn, it->doc)) {
...
void OpObserverImpl::onUpdate(OperationContext* opCtx, const OplogUpdateEntryArgs& args) {
    MONGO_FAIL_POINT_BLOCK(failCollectionUpdates, extraData) {
        ...
        if (!collElem || args.nss.ns() == collElem.String()) {
            uasserted(40654,
                      str::stream()
                          << "failCollectionUpdates failpoint enabled, namespace: " << args.nss.ns()
                          << ", update: " << args.update << " on document with " << args.criteria);
        }
    }
