...
public:
    ...
    ShardConnection(const ConnectionString& connectionString,
                    const std::string& ns,
                    std::shared_ptr<ChunkManagerWithLock> manager = nullptr);
    ...
    std::shared_ptr<ChunkManagerWithLock> getManager() const {
        return _manager;
    }
...
private:
    ...
    const std::shared_ptr<ChunkManagerWithLock> _manager;
